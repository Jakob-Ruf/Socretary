...
public class DailyService extends Service
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		...
		FragmentListener callback = (FragmentListener) ApplicationContext.getActivity();
		callback.loadGeofences(Integer.valueOf(sp.getString("distance", "500")));
		...
		if (pMessageContainer.size() > 1)
		{
			...
			Notification n = new Notification.Builder(this).setContentTitle("Socretary").setContentText("Du hast einige neue Benachrichtigungen (" + pMessageContainer.size() + ")").setSmallIcon(R.drawable.abc_btn_radio_material).setContentIntent(pIntent).setAutoCancel(true).addAction(R.drawable.abc_btn_radio_material, "Nachsehen", pIntent).build();
		...
		else if (pMessageContainer.size() == 1)
		{
			...
			if (pNumContainer.get(0) != null)
			{
				...
				Notification n = new Notification.Builder(this).setContentTitle("Socretary").setContentText(pMessageContainer.get(0)).setSmallIcon(R.drawable.abc_btn_radio_material).setContentIntent(pIntent).setAutoCancel(true).addAction(R.drawable.abc_btn_radio_material, "Anrufen", pCallIntent).addAction(R.drawable.abc_btn_radio_material, "Simsen", pSmsIntent).build();
	...
	private void doaBirthdayCheck()
	{
		...
		if (lContacts.size() > 0)
		{
			...
			for (int i = 0; i < lContacts.size(); i++)
			{
				...
				if ((jodaBirthday.getMonthOfYear() == jodaToday.getMonthOfYear()) && (jodaBirthday.getDayOfMonth() == jodaToday.getDayOfMonth()))
				{
