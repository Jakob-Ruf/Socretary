...
//        db.execSQL(DatabaseContract.MessageForContact.DROP);
	...
	private long insertEncounterGeneral(Encounter encounter, int type)
	{
		...
		int result = db.update(DatabaseContract.ContactEntry.TABLE_NAME, values1, selection, selectionArgs);
		if (result > 0)
		{
	...
	public ArrayList<Contact> getContactList()
	{
		...
		Cursor c;
		c = db.query(DatabaseContract.ContactEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
	...
	public ArrayList<AutomatedMessage> getAutoTextList()
	{
		...
		Cursor c = db.query(DatabaseContract.AutomatedMessage.TABLE_NAME, projection, null, null, null, null, null);
	...
	public Contact getContact(long id)
	{
		...
		Cursor c;
		c = db.query(DatabaseContract.ContactEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, null);
	...
	private long getTimestampOfLatestEncounterForId(long personid)
	{
		...
		Cursor c = db.query(DatabaseContract.EncounterEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder, limit);
		if (c.getCount() > 0)
		{
	...
	public boolean deleteContact(long person_id)
	{
		...
		int success = db.delete(DatabaseContract.ContactEntry.TABLE_NAME, selection, selectionArgs);
	...
	public boolean deleteEncounter(long encounterId)
	{
		...
		int count = db.delete(DatabaseContract.EncounterEntry.TABLE_NAME, selection, selectionArgs);
		return (count != 0);
	}
	...
	private ArrayList<Encounter> getEncounterList(String selection, String[] selectionArgs)
	{
		...
		Cursor c = db.query(DatabaseContract.EncounterEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
		if (c.getCount() > 0)
		{
			Log.d(LOG_CALLER, "GetEncounterList: " + c.getCount() + " Einträge gefunden");
			c.moveToFirst();
